...
const updateIndex = async (_event?: string, path?: string) => {
    if (path === 'index.ts') {
        return;
    }
    const dirs = (await readdir(examples, { withFileTypes: true })).
        ...
        map(i => i.name);
    const sources = dirs.map(d => readFile(`${examples}/${d}/source.tsx`, { encoding: 'utf8' }));
    const compiled = dirs.map(d => readFile(`${examples}/${d}/compiled.ts`, { encoding: 'utf8' }));
    const imports = dirs.
    ...
    await writeFile(`${examples}/index.ts`,
