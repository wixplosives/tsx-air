...
const toString = async (content: Promise<string>) => {
    try {
        return (await content) ? JSON.stringify(await content) : `''`;
    } catch (e) {
        return ``;
    }
};
...
const updateIndex = async (_event?: string, filePath?: string) => {
    ...
    const readme = dirs.map(d => safeRead(`${d}/readme.md`));
    ...
    readme: ${ await toString(await readme[index] ? Promise.resolve(convertor.makeHtml(await readme[index])) : Promise.resolve(''))}
}`);
