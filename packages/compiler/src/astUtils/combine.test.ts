...
describe('combineVisitors', () => {
    ...
    it('should execute all the visitors for all nodes, once per', () => {
        ...
        expect(visited.filter(n => n.node === nodes[0]).map(i => i.note)).to.deep.equal(['1', '2', '3']);
        expect(visited.filter(n => n.node === nodes[1]).map(i => i.note)).to.deep.equal(['1', '2', '3']);
        expect(visited.filter(n => n.node === nodes[2]).map(i => i.note)).to.deep.equal(['1', '2', '3']);
        expect(visited.filter(n => n.node === nodes[3]).map(i => i.note)).to.deep.equal(['1', '2', '3']);
    });
