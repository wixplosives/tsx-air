...
export class SchemaViewRegistry {
    ...
    public getViewForSchema(schema: Schema, variant?: string): SchemaView {
        const match = variant ? this.findView(schema, variant) || this.findView(schema) : this.findView(schema);
    ...
    private findView(schema: Schema, variant?: string) {
        return this.views.find(view => view.variant === variant && view.predicate(schema));
    }
}
