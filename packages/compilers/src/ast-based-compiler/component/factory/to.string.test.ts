...
describe('generateToString', () => {
    const toStringOf = (compDef:CompDefinition, root=0) => evalAst(generateToString(compDef.jsxRoots[root], compDef));
    it('generates a toString method based on the used props and state', () => {
        ...
        expect(toStringOf(comps.WithVolatile)({ p: 2 }), 'WithVolatile')
            .to.equal(`<div dir="ltr" lang="2"></div>`);
    });
