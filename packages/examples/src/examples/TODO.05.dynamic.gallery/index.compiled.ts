    ...
    let div1 = /** @class */ (() => {
        class div1 extends Fragment {
            updateView($ch) {
                ...
                const $b = this.changesBitMap
                if ($ch & ($b["state.images"]))
                    TSXAir.runtime.updateExpression(this.ctx.expressions[0], deleteButton(img.src))
            ...
            <!--X-->${TSXAir.runtime.toString(deleteButton(img.src))}<!--X-->
        </div>`);
            }
            hydrate(_, t) {
                let { deleteButton } = this.volatile;
...
}
{
    let div2 = /** @class */ (() => {
        class div2 extends Fragment {
            updateView($ch) {
                ...
                if ($ch & ($b["props.baseUrl"] | $b["state.images"]))
                    TSXAir.runtime.updateExpression(this.ctx.expressions[0], state.images.map((img, i) => VirtualElement.fragment("1", Gallery.div1, this)))
                if ($ch & ($b["state.images"]))
                    TSXAir.runtime.updateExpression(this.ctx.expressions[1], deleteButton(img.src))
                if ($ch & ($b["props.baseUrl"]))
                    this.ctx.elements[1].setAttribute("src", `/${props.baseUrl}/${img}`)
                if ($ch & ($b["state.images"]))
                    this.ctx.elements[1].setAttribute("alt", `image ${i + 1}/${state.images.length}`)
        ...
        <!--X-->${TSXAir.runtime.toString(state.images.map((img, i) => VirtualElement.fragment("1", Gallery.div1, this)))}<!--X-->
    </div>`);
            }
            hydrate(_, t) {
                const { state } = this.state;
