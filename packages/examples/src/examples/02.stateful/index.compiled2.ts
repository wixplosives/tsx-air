import { Component, VirtualElement, CompFactory, Fragment, Factory, Store } from '@tsx-air/framework';
import { TSXAir, store } from '@tsx-air/framework';
// tslint:disable
type AllowedKeys = Exclude<string, '$bits' | '$data' | '$listeners' | '$subscribe' | '$unsubscribe' | '$dispatch'>;

export type CompiledStore<T extends Record<AllowedKeys, any> = any> = {
    $bits: {
        ...
        class div0 extends Fragment {
            updateView() {
                ...
                if (stateChanged & state.$bits.a) {
                    TSXAir.runtime.updateExpression(this.ctx.expressions[0], state.a);
                }
                if (stateChanged & state.$bits.b) {
                    TSXAir.runtime.updateExpression(this.ctx.expressions[1], state.b);
                }
                if (stateChanged & state.$bits.changeCount) {
                    TSXAir.runtime.updateExpression(this.ctx.expressions[2], state.changeCount);
                }
            }
            toString() {
                const { state } = this.state;
            ...
            <div class="btn" x-da="!">
                <!--X-->${TSXAir.runtime.toString(state.a)}<!--X-->
            </div>
            <div class="btn" x-da="!">
                <!--X-->${TSXAir.runtime.toString(state.b)}<!--X-->
            </div>
            <div class="changeCount">View rendered <!--X-->${TSXAir.runtime.toString(
                state.changeCount
            )}<!--X--> times</div>
            
            <div class="volatile">volatile variable is still at <!--X-->${TSXAir.runtime.toString(
                volatile
            )}<!--X--></div>
        </div>`);
            }
