...
import { BaseView, defaultSchemaViewRegistry } from '@wixc3/schema-views/src';
...
export function getTypescriptSchema(source: string, exportName?: string) {
    ...
    if (exportName) {
        return moduleSchema.properties![exportName] || moduleSchema.definitions![exportName];
    }
    return moduleSchema;
}
...
export function renderSchema(schema: any): Element {
    const html = renderToStaticMarkup(
        <BaseView schema={schema} schemaRegistry={new Map()} viewRegistry={defaultSchemaViewRegistry} />
    );
